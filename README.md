[index (3).html](https://github.com/user-attachments/files/25828124/index.3.html)
<!DOCTYPE html>
<html lang="ja">
<head>
<meta charset="UTF-8">
...
const INIT_SPEAKERS = [
  { id:"s1", chapterId:"kawaguchi",     speakerName:"山田 太郎", company:"株式会社山田商事",     role:"代表取締役",    seminarDate:"2026-03-10", topic:"経営と人の和",              status:"confirmed", phone:"090-1234-5678", email:"yamada@example.com",    requestDate:"2026-02-15", notes:"駐車場確保が必要", venue:"川口商工会議所",          lineNotified:true, materialUrl:"https://drive.google.com/file/d/sample1", materialName:"経営と人の和_資料.pdf" },
  { id:"s2", chapterId:"kawaguchi_east",speakerName:"鈴木 花子", company:"鈴木コンサルティング", role:"代表",          seminarDate:"2026-03-11", topic:"人を活かす組織づくり",      status:"confirmed", phone:"080-9876-5432", email:"suzuki@example.com",   requestDate:"2026-02-20", notes:"",                 venue:"川口東センター",          lineNotified:true  },
  { id:"s3", chapterId:"niizashiki",    speakerName:"佐藤 誠",   company:"佐藤建設",             role:"専務取締役",    seminarDate:"2026-03-09", topic:"地域を支える中小企業の使命", status:"pending",   phone:"070-1111-2222", email:"sato@example.com",     requestDate:"2026-02-28", notes:"日程調整中",       venue:"志木コミュニティセンター", lineNotified:false },
  { id:"s4", chapterId:"asaka",         speakerName:"田中 博",   company:"田中医院",             role:"院長",          seminarDate:"2026-03-13", topic:"健康経営のすすめ",          status:"confirmed", phone:"048-222-3333", email:"tanaka@example.com",   requestDate:"2026-02-10", notes:"",                 venue:"朝霞産業センター",         lineNotified:false },
  { id:"s5", chapterId:"todawarabi",    speakerName:"伊藤 幸子", company:"株式会社伊藤フーズ",   role:"取締役",        seminarDate:"2026-03-12", topic:"食を通じた地域貢献",        status:"confirmed", phone:"048-333-4444", email:"ito@example.com",      requestDate:"2026-02-18", notes:"資料50部印刷依頼", venue:"蕨市民会館",              lineNotified:true  },
  { id:"s6", chapterId:"kawaguchi",     speakerName:"渡辺 正樹", company:"渡辺不動産",           role:"代表取締役社長", seminarDate:"2026-03-17", topic:"不動産から見る地域の未来",  status:"pending",   phone:"090-5555-6666", email:"watanabe@example.com", requestDate:"2026-03-01", notes:"3月上旬確認",      venue:"川口商工会議所",          lineNotified:false },
];
...
function DocSection({ title, color, children }) {
  ...
  return (
    <div style={{ marginBottom:12 }}>
      <div style={{ fontSize:11, fontWeight:700, color:"#fff", background:c, padding:"3px 10px", marginBottom:0, letterSpacing:"0.08em" }}>{title}</div>
...
function DocumentView({ speakers, docSpeaker, setDocSpeaker, today }) {
  ...
  return (
    <div>
      ...
      {sp && ch ? (
        {(() => { const st = getSeminarType(sp.seminarType || "ms"); return (
        <div id="print-doc" style={{ background:"#fff", border:"1px solid #CFD8DC", borderRadius:4, padding:"28px 36px", maxWidth:680, margin:"0 auto", boxShadow:"0 2px 12px rgba(0,0,0,.08)", borderTop:`5px solid ${st.color}`, position:"relative", overflow:"hidden" }}>
          {/* コーナーマーク */}
          <div style={{ position:"absolute", top:0, right:0, width:0, height:0, borderStyle:"solid", borderWidth:"0 60px 60px 0", borderColor:`transparent ${st.color} transparent transparent` }} />
          <div style={{ position:"absolute", top:6, right:4, fontSize:11, fontWeight:700, color:"#fff", zIndex:2, lineHeight:1.2, textAlign:"right" }}>{st.short}</div>
          {/* ヘッダー */}
          <div style={{ textAlign:"center", marginBottom:16 }}>
            <div style={{ fontSize:11, color:"#546E7A", marginBottom:4 }}>倫理法人会　南部地区事務局</div>
            <div style={{ display:"inline-block", fontSize:10, fontWeight:700, color:st.color, border:`1px solid ${st.color}`, padding:"2px 12px", marginBottom:6, letterSpacing:"0.1em" }}>{st.label}</div>
            <h2 style={{ margin:0, fontSize:18, fontWeight:800, color:"#1a1a1a", letterSpacing:"0.04em" }}>講師依頼確認書</h2>
            <div style={{ height:1, background:`linear-gradient(to right, ${st.color}, #ccc, transparent)`, margin:"8px 0 4px" }} />
            <div style={{ fontSize:11, color:"#90A4AE" }}>受付日：　　　　年　　月　　日　　担当：</div>
          </div>
...
function SpeakerForm({ initial, onSave, onClose }) {
  const blank = { chapterId:"kawaguchi", speakerName:"", company:"", role:"", seminarDate:"", topic:"", status:"pending", phone:"", email:"", requestDate: new Date().toISOString().slice(0,10), notes:"", venue:"", seminarType:"ms" };
      ...
      <div style={{ ...MOD, maxWidth:560 }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginTop:14 }}>
          {[
            ...
            { l:"会社名",    k:"company",     t:"text",  p:"株式会社○○" },
            { l:"役職",      k:"role",        t:"text",  p:"代表取締役" },
