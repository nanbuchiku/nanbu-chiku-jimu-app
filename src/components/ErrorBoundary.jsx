      ...
      return (
        <div role="alert" style={{ display:"flex", alignItems:"center", justifyContent:"center", minHeight:"100vh", background:"#F0F2F5", flexDirection:"column", gap:16, padding:24 }}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            ...
            <button style={{ background:"#ECEFF1", color:"#546E7A", border:"none", borderRadius:8, padding:"10px 24px", fontSize:"clamp(13px,1.8vw,16px)", cursor:"pointer", fontWeight:600 }} onClick={() => this.setState({ error: null, errorInfo: null })}>閉じて続ける</button>
          </div>
          {process.env.NODE_ENV === 'development' && this.state.errorInfo && (
            <details style={{ maxWidth:600, fontSize:"clamp(12px,1.4vw,14px)", color:"#90A4AE", background:"#fff", padding:12, borderRadius:6, cursor:"pointer" }}>
